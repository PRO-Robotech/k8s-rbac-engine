...
// NewREST returns a REST handler for subjectpermissionsviews. The engine is
// expected to already have a ReportLookup configured when applicable.
...
func (r *REST) New() runtime.Object { return &rbacgraph.SubjectPermissionsView{} }

func (r *REST) Destroy() {}

func (r *REST) NamespaceScoped() bool { return false }

func (r *REST) GetSingularName() string { return "subjectpermissionsview" }
...
func (r *REST) Create(ctx context.Context, obj runtime.Object, _ rest.ValidateObjectFunc, _ *metav1.CreateOptions) (runtime.Object, error) {
	view, ok := obj.(*rbacgraph.SubjectPermissionsView)
	if !ok {
	...
	if r.authzResolver != nil {
		...
		scope, err := r.authzResolver.Resolve(ctx, userInfo, collectSubjectScopeNamespaces(snapshot))
		if err != nil {
	...
	view.Status = r.engine.QuerySubjectPermissions(snapshot, view.Spec, r.indexer.DiscoveryCache())
	view.CreationTimestamp = metav1.Now()

	return view, nil
}

// collectSubjectScopeNamespaces returns the unique namespaces touched by
// roles and bindings in the snapshot. Used to seed the scope resolver's
// per-namespace access check.
func collectSubjectScopeNamespaces(s *indexer.Snapshot) []string {
